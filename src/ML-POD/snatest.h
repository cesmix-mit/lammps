...
#ifndef LMP_SNATEST_H
#define LMP_SNATEST_H

#include "pointers.h"

namespace LAMMPS_NS {

struct SNATEST_ZINDICES {
  int j1, j2, j, ma1min, ma2max, mb1min, mb2max, na, nb, jju;
};

struct SNATEST_BINDICES {
  int j1, j2, j;
};

class SNATEST : protected Pointers {

 public:
  SNATEST(LAMMPS *, double, int, double, int, int, int, int, int, int, int);

  SNATEST(LAMMPS *lmp) : Pointers(lmp){};
  ~SNATEST() override;
 ...
 private:
  ...
  SNATEST_ZINDICES *idxz;
  SNATEST_BINDICES *idxb;
